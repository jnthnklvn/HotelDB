...
public class HotelDB {
    ...
    protected static void adcConsultas() {
        if (lista1.size() <= 0) {
            ...
            lista2.add("select * from hotel.pessoa p where p.data_nascimento = "
                    + "(select min(data_nascimento) from hotel.pessoa p1 "
                    + "join hotel.cliente c on(p1.cpf=c.pessoa_cpf));");
            lista2.add("select p_nome, sobrenome, email from hotel.pessoa p1 "
            ...
            lista2.add("select p_nome, sobrenome, sq.num_registro, sq.s from hotel.pessoa "
